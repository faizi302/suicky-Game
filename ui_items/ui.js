...
export default class UI {
    constructor(canvas, sceneManager) {
        ...
        // Health bar — managed externally by game.js (set to player.health each frame)
        // Never reset here; only reset in game.reset()
    ...
    update(dt) {
        ...
        // Warning pulse animation at low health
        if (this.playerHealth <= 25 && this.playerHealth > 0) {
            const pulseSpeed = dt / 280;
            this._warningPulse += pulseSpeed * this._warningDir;
            if (this._warningPulse >= 1) { this._warningPulse = 1;  this._warningDir = -1; }
            else if (this._warningPulse <= 0) { this._warningPulse = 0; this._warningDir = 1; }
        } else {
            this._warningPulse = 0;
            this._warningDir   = 1;
        }
    ...
    // Returns true when the settings dropdown (or any UI overlay) is consuming input.
    // GamePauseScene uses this to suppress its own click handling while the dropdown is open.
    isConsumingInput() {
    ...
    _drawHUD(ctx, W, H) {
        ...
        // ── Health bar — below the top cards ────────────────────────────────
        const healthBarY = safeTop + cardH + gap;
        this._drawHealthBar(ctx, leftX, healthBarY, leftW + gap + scoreW, isSmall ? 12 : isMedium ? 14 : 16);
        ...
        // Clicking gear TOGGLES the settings dropdown.
        // ESC / P key (handled by GamePlayScene._handleKeyDown) opens the pause scene.
        this._drawImgBtn(ctx, this.imgSettings, gearX, gearY, gearS, gearS, this._hover(gearX, gearY, gearS, gearS));
    ...
    _drawHealthBar(ctx, x, y, totalWidth, barH) {
        ...
        const isMid  = health <= 50 && health > 25;

        const radius = Math.round(barH / 2);
        const labelW = Math.round(totalWidth * 0.18);
        const barX   = x + labelW;
        ...
        let barColor;
        if (isLow) {
            const r1 = [220, 30, 30], r2 = [255, 80, 80];
            const t  = this._warningPulse;
            barColor = `rgb(${Math.round(r1[0]+(r2[0]-r1[0])*t)},${Math.round(r1[1]+(r2[1]-r1[1])*t)},${Math.round(r1[2]+(r2[2]-r1[2])*t)})`;
        } else if (isMid) {
            barColor = '#e8a020';
        } else {
            barColor = '#2ecc40';
        ...
        // Red glow at low health
        if (isLow) {
            ctx.save();
            ctx.shadowColor = '#ff2020';
            ctx.shadowBlur  = 10 + this._warningPulse * 10;
            ctx.strokeStyle = `rgba(255,50,50,${0.3 + this._warningPulse * 0.55})`;
            ctx.lineWidth   = 2.5;
            this._rr(ctx, barX, y, barW, barH, radius);
            ctx.stroke();
            ctx.restore();
        ...
        // "!" danger icon
        if (isLow) {
            ctx.fillStyle    = `rgba(255,80,80,${0.6 + this._warningPulse * 0.4})`;
            ctx.font         = `bold ${Math.round(barH * 1.3)}px Arial`;
            ctx.textAlign    = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText('!', barX + barW + 5, y + barH / 2);
        }
    ...
    _drawDropdown(ctx, W, H) {
        ...
        defs.forEach((d, i) => {
            ...
            if (d.id === 'sound' && d.img?.complete && d.img.naturalWidth > 0) {
                ...
                ctx.drawImage(d.img, (this.soundOn ? 0 : 1) * fw, 0, fw, d.img.naturalHeight, gx, y, bs, bs);
            } else if (d.id === 'fullscreen' && d.img?.complete && d.img.naturalWidth > 0) {
                const fw = d.img.naturalWidth / 2;
                ctx.drawImage(d.img, (this.isFullscreen ? 1 : 0) * fw, 0, fw, d.img.naturalHeight, gx, y, bs, bs);
            ...
            ctx.restore();
            if (t > 0.4) this._dropBtns.push({ id: d.id, x: gx, y, w: bs, h: bs });
        });
    }
    ...
    _drawHelpPanel(ctx, W, H) {
        ...
        const bW = Math.min(W * 0.88, 520), bH = Math.min(H * 0.88, 370);
        const bX = (W - bW) / 2, bY = (H - bH) / 2;

        ctx.translate(W / 2, H / 2);
        ctx.scale(0.92 + this.overlayAnim * 0.08, 0.92 + this.overlayAnim * 0.08);
        ctx.translate(-W / 2, -H / 2);
        ...
        if (this.helpPage === 0) {
            ...
            if (kImg?.complete && kImg.naturalWidth > 0) {
                const fW = Math.round(kImg.naturalWidth / 2), fH = Math.round(kImg.naturalHeight / 4);
            ...
            if (sImg?.complete && sImg.naturalWidth > 0) {
                const dw = Math.round(160 * sc), dh = Math.round(46 * sc);
                ctx.drawImage(sImg, 0, 0, sImg.naturalWidth/2, sImg.naturalHeight, W/2 - dw/2, bY + bH*0.65, dw, dh);
            }
            ctx.fillText('JUMP (Spacebar / ↑)', W / 2, bY + bH * 0.87);
        } else {
            ctx.fillStyle = '#fff';
            ctx.font = `${Math.round(15 * sc)}px Arial`;
            ['ESC or P → Pause', 'F → Toggle Fullscreen', 'Arrow Keys / WASD → Move', 'Space / ↑ → Jump']
                .forEach((l, i) => ctx.fillText(l, W / 2, bY + bH * (0.30 + i * 0.16)));
        }

        const arW = Math.round(34*sc), arH = Math.round(44*sc);
        const arLx = bX - arW - 8, arRx = bX + bW + 8, arY = (H - arH) / 2;

        ctx.save();
        ctx.translate(arLx + arW, arY); ctx.scale(-1, 1);
        ...
        const tkS = Math.round(48*sc), tkX = bX + bW - tkS - 8, tkY = bY + bH - tkS - 8;
    ...
    _drawExitConfirm(ctx, W, H) {
        ...
        const bW = Math.min(W*0.85, 460), bH = Math.min(H*0.75, 260);
        const bX = (W-bW)/2, bY = (H-bH)/2;
        ctx.translate(W/2, H/2);
        ctx.scale(0.92 + this.overlayAnim*0.08, 0.92 + this.overlayAnim*0.08);
        ctx.translate(-W/2, -H/2);
        ...
        const btnS = Math.round(56*sc), btnY2 = bY + bH - btnS - Math.round(14*sc);
        const noX  = W/2 - btnS - Math.round(10*sc), yesX = W/2 + Math.round(10*sc);

        if (this.imgButExit?.complete) ctx.drawImage(this.imgButExit, noX,  btnY2, btnS, btnS);
        else this._fallbackBtn(ctx, noX, btnY2, btnS, btnS, '✕', false);
        if (this.imgButYes?.complete)  ctx.drawImage(this.imgButYes,  yesX, btnY2, btnS, btnS);
        else this._fallbackBtn(ctx, yesX, btnY2, btnS, btnS, '✓', false);

        this._exitBtns = {
            no:  { x: noX,  y: btnY2, w: btnS, h: btnS },
            yes: { x: yesX, y: btnY2, w: btnS, h: btnS },
        };
        ctx.restore();
    }
    ...
    _drawGameOver(ctx, W, H) {
        ...
        ctx.scale(0.88 + this.overlayAnim*0.12, 0.88 + this.overlayAnim*0.12);
        ctx.translate(-cx, -H/2);
        ...
        ctx.fillText(winMode ? 'CONGRATULATIONS!' : 'GAME OVER', cx, boxY + boxH*0.18);

        ctx.font = `bold ${Math.round(boxH*0.10)}px Arial Black, Arial`;
        ctx.fillText(`SCORE: ${this.score || 0}`,       cx, boxY + boxH*0.43);
        ctx.fillText(`TOTAL SCORE: ${(this.totalScore||0) + (winMode ? 0 : this.score||0)}`, cx, boxY + boxH*0.61);

        const btnSize  = Math.round(Math.min(boxW*0.17, 92));
        const btnGap   = Math.round(btnSize*0.33);
        const btnCount = winMode ? 3 : 2;
        const startX   = cx - (btnCount*btnSize + (btnCount-1)*btnGap) / 2;
        ...
        if (winMode) btns.push({ id: 'next', img: this.imgNext, x: startX + (btnSize+btnGap)*2, y: btnY, w: btnSize, h: btnSize });

        this._gameOverBtns = {};
        for (const b of btns) {
            this._drawOverlayButton(ctx, b.img, b.x, b.y, b.w, b.h, this._hover(b.x,b.y,b.w,b.h), b.id);
    ...
    _drawOverlayButton(ctx, img, x, y, w, h, hovered, id) {
        ...
        else this._fallbackBtn(ctx, x, y, w, h, id==='home'?'⌂':id==='restart'?'↺':'▶', hovered);
        ctx.restore();
    }
    ...
    _drawCard(ctx, x, y, w, h) {
        ...
        if (img?.complete && img.naturalWidth > 0) { ctx.drawImage(img, x, y, w, h); return; }
        ctx.save();
    ...
    _fallbackBtn(ctx, x, y, w, h, label, hov) {
        ...
        ctx.strokeStyle = '#c8872a'; ctx.lineWidth = 2;
        this._rr(ctx, x, y, w, h, 8); ctx.fill(); ctx.stroke();
        ctx.fillStyle = '#fff';
    ...
    _onMouseMove(e)      { const p = this._getPos(e.clientX, e.clientY); this.mx = p.x; this.my = p.y; }
    _onMouseClick(e)     { if (performance.now() - this._lastTouchAt < 450) return; const p = this._getPos(e.clientX, e.clientY); this._processClick(p.x, p.y); }
    _handleTouchStart(e) { if (!e.touches.length) return; const t = e.touches[0]; const p = this._getPos(t.clientX, t.clientY); this.mx = p.x; this.my = p.y; e.preventDefault(); }
    _handleTouchEnd(e)   { if (!e.changedTouches.length) return; const t = e.changedTouches[0]; const p = this._getPos(t.clientX, t.clientY); this.mx = p.x; this.my = p.y; this._lastTouchAt = performance.now(); e.preventDefault(); this._processClick(p.x, p.y); }
    _handleTouchCancel(e){ this._lastTouchAt = performance.now(); e.preventDefault(); }

    _processClick(mx, my) {
        const hit = b => b && mx >= b.x && mx <= b.x+b.w && my >= b.y && my <= b.y+b.h;

        // ── Game-over overlay: only these buttons respond ────────────────────
        if ((this.gameOver || this.showGameOver) && this._gameOverBtns) {
            ...
            if (hit(this._gameOverBtns.next))    { this.pendingNext = true; return; }
            return;
        }
        ...
        // ── Dropdown buttons (only active when dropdown is open) ─────────────
        if (this.settingsOpen && this._dropBtns.length > 0) {
            for (const b of this._dropBtns) {
                if (hit(b)) {
                    this._handleDropBtn(b.id);
                    return;
                }
            }
            // Clicked outside the dropdown area — close it
        ...
        if (hit(this._gearBtn)) {
            if (!this.gameOver && !this.showGameOver && !this.win) {
    ...
    // Handle each dropdown item action
    _handleDropBtn(id) {
        switch (id) {
            case 'exit':
                this.settingsOpen = false;
            ...
            case 'sound':
                ...
                this._applySound();
                // Keep dropdown open so player can see the toggled state
                break;
            case 'fullscreen':
                this._toggleFS();
                // Keep dropdown open
    ...
    _goHome()    { if (this.sceneManager?.setScene) this.sceneManager.setScene('menu'); else window.location.reload(); }
    _restart()   { this.pendingRestart = true; }
}