...
const INVINCIBILITY_MS     = 900;  // ms of invincibility after taking a hit

// Exported so game.js can import them
export const ENEMY_DAMAGE  = 50;
export const KNIFE_DAMAGE  = 25;
...
export default class Player {
    constructor(game) {
        ...
        // ── Health system ─────────────────────────────────────────────────────
        // health starts at MAX and is ONLY reset in game.reset(), NOT on death.
        // When dead, health stays at 0 until the game restarts.
        this.health          = MAX_HEALTH;
        this.invincibleTimer = 0;   // ms remaining
        this._flashTimer     = 0;   // used for blink effect
    }

    /**
     * Reduce health by amount.  Triggers invincibility frames.
     * If health reaches 0, calls die().
     * Does NOT reset health — that happens only in game.reset().
     */
    takeDamage(amount) {
        ...
        this.invincibleTimer = INVINCIBILITY_MS;
        this._flashTimer     = INVINCIBILITY_MS;

        if (this.health <= 0) {
            this.die();
        } else {
            // Soft hit sound (quieter than death)
            playSound('snd_die', 0.45, 0, false);
        }
    }

    die() {
        if (TEST_MODE_NO_DEATH) return;
        if (!this.alive) return;

        this.alive  = false;
        this.health = 0;      // ensure it reads 0 while the death animation plays
        this.vx     = 0;
        this.vy     = -7.2;
        this.currentPlatform = null;
    ...
    onCoinCollected() { playSound('snd_coin',  0.75, 40, true); }
    onKeyCollected()  { playSound('snd_key',   0.85, 40, true); }

    onEnemyKilled(type = 0) {
        const id = `snd_enemy_${type}_death`;
        if (document.getElementById(id)) playSound(id, 0.8, 50, true);
        else                              playSound('snd_enemy_0_death', 0.8, 50, true);
    ...
    onLockTremble() { playSound('snd_lock_tremble', 0.55, 220, true); }

    update(keys, dt, map, movingPlatforms = []) {
        // Tick invincibility / flash timers
        ...
        if (this.currentLadder && (wantsUp || wantsDown)) this.isClimbing = true;
        ...
        if (right) { this.vx += this.speed * (dt * 0.1); this.facing = 1;  this.isMoving = true; }
        if (left)  { this.vx -= this.speed * (dt * 0.1); this.facing = -1; this.isMoving = true; }
        ...
        if (this.x < this.tileSize) { this.x = this.tileSize; this.vx = 0; }

        // Fall into gap = instant kill (sets health to 0 then calls die())
    ...
    resolveMovingPlatformCollision(platforms = [], prevTop, prevBottom) {
        ...
        for (const p of platforms) {
            const overlapX = this.x + this.width > p.x + 2 && this.x < p.x + p.width - 2;
            if (!overlapX) continue;

            const wasAbove   = prevBottom <= p.y + 6;
            const crossedTop = currBottom >= p.y && currBottom <= p.y + p.height + 14;
            if (this.vy >= 0 && wasAbove && crossedTop) {
                this.y = p.y - this.height; this.vy = 0;
                this.onGround = true; this.currentPlatform = p; landed = true; continue;
            }

            const wasBelow      = prevTop >= p.y + p.height - 4;
            const crossedBottom = currTop <= p.y + p.height && currTop >= p.y - 14;
            if (this.vy < 0 && wasBelow && crossedBottom) {
                this.y = p.y + p.height; this.vy = 0; continue;
            }
        }

        if (!landed) {
            const ok = this.currentPlatform &&
            ...
            if (!ok) this.currentPlatform = null;
        }
    }

    _handleClimb(keys) {
        if (!this.currentLadder) { this.stopClimbing(); return; }
        ...
        this.x  = ladder.x + ladder.width * 0.5 - this.width * 0.5 - 7;
        this.vx = 0;
        this.vy = 0;
        ...
        if (this.y <= topExitY)    { this.y = topExitY;    this.vy = 0; this.stopClimbing(); this.onGround = false; return; }
        if (this.y >= bottomExitY) { this.y = bottomExitY; this.vy = 0; this.stopClimbing(); this.onGround = true;  return; }
        if (left || right)         { this.stopClimbing(); return; }
    ...
    _updateDeath(dt) {
        ...
        while (this.deadTimer >= DIE_FRAME_MS) {
            ...
            if (this.deadFrame < NORMAL_FRAME_COUNT - 1) this.deadFrame++;
            else { this.deadDone = true; break; }
        }
    }
    ...
    draw(ctx, camera) {
        ...
        // Blink effect during invincibility (after taking damage, while alive)
        if (this.invincibleTimer > 0 && this.alive) {
            const blinkRate = 120;
            const phase = Math.floor(this._flashTimer / blinkRate) % 2;
            ctx.globalAlpha = phase === 0 ? 0.3 : 1.0;
        }
        ...
        if (this.anim === 'dead' || this.anim === 'climb') {
            ...
            if (sheet && sheet.complete && sheet.naturalWidth > 0) {
                ...
                if (flip) {
                    ctx.translate(drawX + DRAW_W, drawY); ctx.scale(-1, 1);
        ...
        if      (this.anim === 'run')  img = this.frames[this.frameIndex] || this.frames[0];
        else if (this.anim === 'jump') img = this.frames[5] || this.frames[0];

        if (!img || !img.complete) { ctx.restore(); return; }
